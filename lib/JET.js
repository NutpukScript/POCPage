/* JET: version = 1.11.2, released date = 16/03/2015 */
...
_JetUtil.prototype.callSubscriptions = function(obj, key, args, preventDefault) {
    if (key in obj) {
        var res;
        var hlist;
        if(preventDefault || obj['default'] == null) {
            hlist = obj[key];
        }else hlist = obj[key].concat(obj['default']);
...
_JetUtil.prototype.deepCopy = function(obj) {
    if (Object.prototype.toString.call(obj) === '[object Array]') {
        var newArray = [],
            ...
            len = obj.length;
        for (; i < len; i++) {
            newArray[i] = _JetUtil.prototype.deepCopy(obj[i]);
        }
        return newArray;
    }
    if (typeof obj === 'object') {
        var newObj = {},
            j;
        for (j in obj) {
            newObj[j] = _JetUtil.prototype.deepCopy(obj[j]);
        }
        return newObj;
    }
    return obj;
...
_JetUtil.prototype.escapeCDATA = function(input) {
    //find first of "<![CDATA["
    //find last of "\]\]\>"
    var m = input.indexOf("<![CDATA[");
    var n = input.lastIndexOf("\]\]\>");
    if (m < 0 || n < 0) return input;
    return input.substring(0, m + 8) + (n) + input.substring(m + 8, n) + input.substring(n + 3);
};

_JetUtil.prototype.unescapeCDATA = function(input) {
    //find first of "<![CDATA["
    var m = input.indexOf("<![CDATA");
    var n = input.indexOf("[", m + 8);
    if (m < 0 || n < 0) return input;
    if (n === m + 8) return input;
    var num_str = input.substring(m + 8, n);
    var len = num_str.length;
    var num = parseInt(num_str);
    return input.substring(0, m + 8) + input.substring(m + 8 + len, num + len) + "\]\]\>" + input.substring(num + len);
};

_JetUtil.prototype.wrapCData = function(input) {
    if (input == null) return input;
    return "<![CDATA[" + this.escapeCDATA(input) + "\]\]\>";
};
...
_JetUtil.prototype.unwrapCData = function(input) {
    ...
    var m = input.indexOf("<![CDATA[");
    var n = input.lastIndexOf("\]\]\>");
    if (m === 0 && n === input.length - 3) return this.unescapeCDATA(input.substring(9, input.length - 3));
    else return this.unescapeCDATA(input);
};
...
JET = function() {
    ...
    api.extend = function(ver, namespace, declaration) {
        ...
        if (!JET[namespace]) {
            try {
                ...
                _u.warn("Fail to load plugin: " + namespace + " due to " + e.toString());
            }
        }
    };
    ...
    **************************************************/
    ...
    api.getJETProperties = function(name, xmlData) {
        return {
            name: name,
            result: _dataProviders[name].call(JET, xmlData)
        }
    };
    ...
    var _getPersistURL = function() {
        var topUrl = null;
        try {
            topUrl = window.top.location.href;
        } catch (ex) {}
        return JET.PersistURL || topUrl;
    }

    var _getArchiveData = function() {
        var url = _getPersistURL();
        var data = {
            data: []
        };
        if (url !== null) {
    ...
    *******************************************************************/
    ...
    api.dragStart = function(data) {
        ...
        // set dnd data
        if (JET.DragAndDrop) JET.DragAndDrop.setDropData(data);
    ...
    *******************************************************************/
    ...
    var _checkSyncDataRequestAllowed = function() {
        if (!_isSyncDataRequestAllowed())
            _u.error("Synchronous requests are not supported by the Container!");
    };
    ...
    *********************************************************************/
    ...
    var _callEventHandler = function(eventObj, preventDefault) {
        var res = _u.callSubscriptions(_eventHandlers, eventObj.name, [eventObj], preventDefault);
    ...
    api.callEventHandler = _callEventHandler;
    ...
    ****************************************************************/
    ...
    api.init = function(initObj) {
        ...
        var _initializeQueue = _u.queue().resolve();
        ...
        var Socket = function(source, originId) {
            this.container = {};
            this.ID = originId;
            var properties = {};
            ...
            this.callAsync = function(data, asyncId) {
                source.postMessage({
                    data: data,
                    method: 'resultEvent',
                    targetId: originId,
            ...
            this.callAny = function(method, data) {
                send(method, data);
            }

            this.container.init = function() {
                //request all sync data
                ...
                request_static_data("description");
                request_static_data("ContainerInformation");
                request_static_data("isActive");

                send("containerRequestData", {
                    name: 'properties'
                });

            }

            this.container.processEvent = function(e) {
                _container.processEvent(e);
            }

            this.container.getData = function(d) {
                if (d.name === 'persistdata') {
                    var fireArchive = function() {
                        send("setData", {
                            name: d.name,
                            data: _u.unwrapCData(_archiveHash[originId])
                        });
                    }
                    if (!api.Loaded) {
                        api.onLoad(fireArchive);
                    } else {
                        fireArchive();
                    }
                } else {
                    var res = _container.getData(d);
                    send("setData", {
                        name: d.name,
                        data: res
                    });
            ...
            this.container.setRequestedData = function(data) {
                var name = data.name;
        ...
        var onPostMessage = function(event) {
            var e = event.data;
            if(!(e.jetClient)) return;
            var listener = JETClients[e.originId];
            if (!listener) {
                listener = new Socket(event.source, e.originId);
                JETClients[e.originId] = listener;
                ClientList.push(listener);
            }

            if (e.asyncId) {
                e.data.async = function(data) {
                    listener.callAsync(data, e.asyncId);
                }
            }

            if (e.method === 'registerWithCCFContainer') {
                listener.container.init();
            } else if (e.method === 'processEvent') {

                if (e.data.name === 'onUnload' || e.data.name === 'persistdata') {
                    _archiveHash[e.originId] = _u.wrapCData(e.data.xmlData);
                    delete JETClients[e.originId];
                    for (var i = 0; i < ClientList.length; ++i)
                        if (ClientList[i].ID === e.originId) ClientList.splice(i, 1);
                } else {
        ...
        if (window.addEventListener) window.addEventListener("message", onPostMessage, false);
        else window.attachEvent("onmessage", onPostMessage);

        /////////////////////////////// Open direct channel ////////////////////////////////

        var DirectClient = function(jet) {
            var properties = {};
            this.ID = jet.ID;

            this.callAny = function(method, data) {
                jet.callEventHandler(data, method === 'default');
            }
            this.setRequestedData = function(data) {
                var name = data.name;
                var result = data.result;
                switch (name) {
                    case 'properties':
        ...
        JET.DirectChannel = function(e, jet) {
            var listener = JETClients[jet.ID];
            if (!listener) {
                listener = new DirectClient(jet)
                JETClients[jet.ID] = listener;
                ClientList.push(listener);
            }

            if (e.method === 'registerWithCCFContainer') {
                listener.setRequestedData(jet.getJETProperties('properties', null));
            } else if (e.method === 'processEvent') {

                if (e.data.name === 'onUnload' || e.data.name === 'persistdata') {
                    _archiveHash[jet.ID] = _u.wrapCData(e.data.xmlData);
                    delete JETClients[jet.ID];
                    for (var i = 0; i < ClientList.length; ++i)
                        if (ClientList[i].ID === jet.ID) ClientList.splice(i, 1);
                } else {
                    _container.processEvent(e.data);
                }

            } else if (e.method === 'getData') {
                if (e.data.name === 'persistdata') {
                    //wrap async
                    if (!api.Loaded) {
                        var oldAsync = e.data.async;
                        e.data.async = function(res) {
                            oldAsync(_u.unwrapCData(_archiveHash[jet.ID]));
                        };
                    } else {
                        var res = _u.unwrapCData(_archiveHash[jet.ID]);
                        if (e.data && e.data.async != null) {
                            e.data.async(res);
                        }
                        return res;
                    }
                } else return _container.getData(e.data);
            }
            return null;
        }
        ...
        _addEventHandler("default", function(data) {
            for (var i = 0; i < ClientList.length; ++i)
                ClientList[i].callAny('default', data);
        });
        ...
        var checkEikonNow = function() {
            return window.EikonNow && window.EikonNow.isEnabled();
        }

        var checkTopNonCrossDomain = function() {
            try {
                window.top.JET;
                if (window.top.JET_USE_POST_MESSAGE) return false;
            } catch (e) {
                return false;
            }
            return true;
        ...
        var checkTopJET = function() {
            if (window.top.JET != null && window.top.JET.Initialized) return true;
            else return false;
        }

        var checkContainerType = function(top_window) {
            var jsAsyncContainer = "registerWithAsyncCCFContainer" in top_window;
            if (jsAsyncContainer || top_window.externalHost != null || ("registerWithJET" in top_window) || ("eikonLinkReady" in top_window))
                return 'initChrome';
            else {
                if ("registerWithCCFContainer" in top_window || (top_window.external && "registerWithCCFContainer" in top_window.external))
                    return 'initKoIE';
                else return 'initUnknown';
            }
        }

        var initChrome = function(top_window) {

            var chromium = new JET.Container.Chromium(
                _u,
                "registerWithAsyncCCFContainer" in top_window,
                _defaultTimeout,
        ...
        var initKoIE = function(top_window) {
            ...
            var container = co.registerWithCCFContainer({
                getData: onGetData,
        ...
        var initTunneling = function(top_window) {
            ...
            var send = function(method, data, listener) {
                if (data && data.async) {
                    ...
                    callbacks[asyncId] = data.async;
                    asyncId = (new Date()).getTime();
                    top_window.postMessage({
                        originId: originId,
                        method: method,
                        data: data,
                        asyncId: asyncId,
            ...
            var receive = function(event) {
                var e = event.data;
                if(!(e.jetHost)) return;
                if (!!e && e.targetId === originId) {
                    switch (e.method) {
                        case 'setData':
                            if (e.data) {
                                clientData[e.data.name] = e.data.data;
                                if (e.data.name === 'persistdata' && onLoaded != null) {
                                    onLoaded(e.data.data);
                                    onLoaded = null;
                                }
                                return true;
                            }
                        ...
                        case 'unsupported':
                            ...
                            // return this.oldContainer.processEvent(e.data, this.ccfInterface);
                            break;
                        case 'resultEvent':
                            callbacks[e.data.asyncId](e.data.data);
                            break;
                        case 'containerRequestData':
                            send("containerGetData", JET.getJETProperties(e.data.name, e.data.xmlData), this);
                            break;
                        default:
                            _callEventHandler(e.data);
                            break;
                    }
                }
            };

            if (window.addEventListener) window.addEventListener("message", receive, true);
            else window.attachEvent("onmessage", receive);

            JET.ContainerType = "Sync";
            _container = {};
            _container.getData = function(data) {
            ...
            _container.processEvent = function(e) {
                send("processEvent", e, this);
                return true;
            }

            send("registerWithCCFContainer", null, this);

            JET.Initialized = true;
            defInitialized.resolve();
        }

        var initSyncTunneling = function(top_window) {
            if(top_window === window || JET.Initialized) {
                defInitialized.resolve();
                return;
            }
            JET.ContainerType = "Sync";
            _container = {};
            _container.getData = function(data) {
                return top_window.JET.DirectChannel({
                    method: "getData",
                    data: data
                }, JET);
            }
            ...
            top_window.JET.DirectChannel({
                method: "registerWithCCFContainer"
            }, JET);
        ...
        var initUnknown = function(top_window) {
        ...
        _initializeQueue.then(function() {
            if (checkEikonNow()) {
                initKoIE(window);
            } else if (!checkTopNonCrossDomain()) {
                initTunneling(window.top);
            } else if (checkTopJET()) {
                initSyncTunneling(window.top);
            } else {
                var containerType = checkContainerType(window.top);
                switch (containerType) {
                    case 'initChrome':
                        initChrome(window.top);
                        break;
                    case 'initKoIE':
                        initKoIE(window.top);
                        break;
                    case 'initUnknown':
                        initUnknown(window.top);
                        return;
                        break;
                }
                window.top.JET = JET;
        ...
        var onPersistCallback = function(persist) {
        ...
        _initializeQueue.then(function() {
            ...
            _getData({
                ...
                async: function(persist){
                    if(persist === '' && (JET.ContainerDescription == null || JET.ContainerDescription.name !== "Thomson Reuters Eikon")){
                        var hasFired = false;
                        JET.subscribe("/navigation/archive_response", function(message){
                            var json = JSON.parse(message);
                            if(json.id === JET.ID && !hasFired) {
                                onPersistCallback(json.persist);
                                hasFired = true;
                            }
                        }, null);
                        JET.publish("/navigation/archive_request", JSON.stringify({id:JET.ID, url: window.location.href}));
                        setTimeout(function(){
                            if(!hasFired) {
                                onPersistCallback('');
                                hasFired = true;
                    ...
                    } else onPersistCallback(persist);
                }
            });
        });
    }
    ...
    var _getDescriptionJson = function (root) {
        ...
        _u.each(root.childNodes, function (node) {
            switch (node.nodeName) {
                ...
                case "WindowInfo":
                    var windowInfo = {};
                    _loadAttr(node, "windowId", false, windowInfo, "windowId");
                    _loadAttr(node, "isFlexViewer", false, windowInfo, "isFlexViewer");

                    if (windowInfo.isFlexViewer !== undefined) {
                        windowInfo.isFlexViewer = (windowInfo.isFlexViewer === "true");
                    }
                    else {
                        _u.warn("Warning: isFlexViewer attribute is not available in WindowInfo node");
                    }
                    ...
                    if (windowInfo.windowId) {
                        ...
                        _u.warn("Warning: windowId attribute is not available in WindowInfo node");
                    }

                    if (windowInfo.isFlexViewer == undefined || windowInfo.windowId == undefined) {
...
JET.Container.Chromium.prototype._asyncContainer = {
    postMessage: function(o) {
        if(this._u) this._u.warn("Jet not loaded, skipping " + o);
    }
};
...
/*Serializing json into xml*/
JET.extend(0, "DragAndDrop", function(_u) {
    ...
    **************************************************************/
    ...
    function callResetDragDrop() {
        if (window.externalHost != null) JET.getData({
            name: "resetDragDrop",
            xmlData: null,
            async: function() {}
        });
